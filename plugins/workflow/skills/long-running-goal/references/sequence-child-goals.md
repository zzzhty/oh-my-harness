...
A fully specified later action may remain Pending in its owner's `Deferred approval gates` under `../components/planning-preflight.md`. Put child gates in the child contract; parent execution or promotion never approves them. Earlier authorized stages follow the existing strict order, and the gated milestone's work waits for actual authorization. This does not add per-child execution approval or permit skipping a blocked child.
