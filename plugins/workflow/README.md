...
- `Runtime hard stops`: the only post-`Ready` long-running-goal conditions that should pause for the user, such as repeated technical impossibility, missing unavailable credentials or source facts, destructive/irreversible/privacy-sensitive/external actions beyond recorded authorization, frozen semantic conflict, or required verifier/subagent/connector failure with no in-plan local next step.
