...
- `Runtime hard stops`: post-`Ready` conditions that block the affected action, such as repeated technical impossibility, required inputs/tools unavailable through authorized recovery, destructive/irreversible/privacy-sensitive/external actions beyond recorded authorization, frozen semantic conflict, or required verifier/subagent/connector failure with no in-plan local next step. A specified future approval gate permits earlier authorized preparation; its owning milestone waits for approval.

Planning preflight requires the grilling and domain-modeling workflow, with sourced completion, valid completed-preflight reuse, or an explicit user skip. Confirmed answers reduce questions; a complete-looking plan does not waive the workflow. Its authorization coverage resolves actual planned permissions through Close, preserves sourced final-review gates, and lets execution reuse still-valid authority across milestones and context transitions. Atomic goals and sequences share the contract; child registers match the tuple while evidence remains with each child. Time estimates are optional and housekeeping stays independently governed. `grill-with-docs` remains the explicit Matt entrypoint; Workflow directly composes its underlying methods.
